...
int get_data_export(struct pe_image *pe, char *name, uint32_t base, void *result)
{
        uint32_t *hack = result;

        get_export(pe, name, result);
...
void *get_export_address(struct pe_image *pe, const char *name)
{
        void *address;
        if (get_export(pe, name, &address) != -1)
                return address;
        return NULL;
}

int get_export(struct pe_image *pe, const char *name, void *result)
{
...
static void *get_dll_init(struct pe_image *pe, char *name)
{
        int i;
        for (i = 0; i < pe->num_pe_exports; i++)
                if ((strcmp(pe->pe_exports[i].dll, name) == 0) &&
                    (strcmp(pe->pe_exports[i].name, "DllInitialize") == 0))
                        return (void *)pe->pe_exports[i].addr;
        return NULL;
}
...
struct pe_image *imported_dlls[1024] = {0};
int num_imported_dlls = 0;

int import(struct pe_image *pe, IMAGE_IMPORT_DESCRIPTOR *dirent, char *dll)
{
        ...
        struct pe_image *dllPe = NULL;

        DBGLINKER("%s imports %s", pe->name, dll);

        // Check if not KERNEL32.DLL
        if (strcmp(dll, "KERNEL32.dll") != 0)
        {
                // Combine "engine/win64/" + dll
                char *dllLocation = malloc(strlen("engine/win64/") + strlen(dll) + 1);
                strcpy(dllLocation, "engine/win64/");
                strcat(dllLocation, dll);

                // Check if already imported
                for (int i = 0; i < num_imported_dlls; i++)
                {
                        // DBGLINKER("checking %d %p %p %s", i, imported_dlls[i], imported_dlls[i]->name, imported_dlls[i]->name);
                        if (strcmp(imported_dlls[i]->name, dll) == 0)
                        {
                                dllPe = imported_dlls[i];
                                DBGLINKER("- found %s in cache at %d", dll, i);
                                break;
                        }
                }
                ...
                if (dllPe == NULL)
                {
                        DBGLINKER("- importing from %s", dllLocation);

                        dllPe = malloc(sizeof(struct pe_image));
                        dllPe->entry = NULL;
                        dllPe->name = dll;

                        if (pe_load_library(dllLocation, &dllPe->image, &dllPe->size) == false)
                        {
                                DBGLINKER("- failed to load %s", dll);
                        }
                        else
                        {
                                imported_dlls[num_imported_dlls] = dllPe;
                                DBGLINKER("- loaded %s cached at %d -> %p", dll, num_imported_dlls, dllPe);
                                num_imported_dlls++;
                                link_pe_images(dllPe, 1);
                        }
                }
        }
        ...
        for (int i = 0; lookup_tbl[i]; i++)
        {
                if (IMAGE_SNAP_BY_ORDINAL(lookup_tbl[i]))
                {
                        ...
                        address_tbl[i] = &ordinal_import_stub;
                        continue;
                }
                else
                {
                ...
                {
                        if (dllPe == NULL || get_export(dllPe, symname, &adr) < 0)
                        {
                                // ERROR("%d -> %p -> %p unknown symbol: %s:%s now unknown_symbol_stub %p", i, address_tbl[i], lookup_tbl[i], dll, symname, &unknown_symbol_stub);
                                // ERROR("unknown symbol: %s:%s", dll, symname);
                                address_tbl[i] = &unknown_symbol_stub;
                                // continue;
                        }
                        else
                        {
                                // DBGLINKER("Found symbol: %s:%s: addr: %p", dll, symname, adr);
...
int fixup_imports(struct pe_image *pe)
{
        int ret = 0;
        ...
        {
                char *name = RVA2VA(pe->image, dirent[i].Name, char *);

                // check if not empty
                if (name[0] == 0)
                {
                        DBGLINKER("empty import name at %d %s", i, pe->name);
                        continue;
                }
...
static int fix_pe_image(struct pe_image *pe)
{
        ...
        // DBGLINKER("mmap image: %d bytes, image base %#x",
...
int link_pe_images(struct pe_image *pe_image, unsigned short n)
{
        ...
        for (i = 0; i < n; i++)
        {
                ...
                bool is64 = pe->opt_hdr->Magic == IMAGE_NT_OPTIONAL_HDR64_MAGIC;
                DBGLINKER("is64: %d", is64);
